...
def disan(rep_tensor, rep_mask, scope=None,
    ...
    with tf.compat.v1.variable_scope(scope or 'DiSAN'):
        with tf.compat.v1.variable_scope('ct_attn'):
        ...
        with tf.compat.v1.variable_scope('sent_enc_attn'):
...
def directional_attention_with_dense(rep_tensor, direction=None, scope=None,
    ...
    with tf.compat.v1.variable_scope(scope or 'directional_attention_%s' % direction or 'diag'):
        ...
        rep_mask = tf.cast(tf.linalg.diag(- tf.ones([sl], tf.int32)) + 1, tf.bool)
...
def bn_dense_layer(input_tensor, hn, bias, bias_start=0.0, scope=None,
    ...
    with tf.compat.v1.variable_scope(scope or 'bn_dense_layer'):
        linear_map = linear(input_tensor, hn, bias, bias_start, 'linear_map',
                            False, wd, keep_prob, is_train)
        if enable_bn:
            linear_map = tf.compat.v1.layers.batch_norm(
